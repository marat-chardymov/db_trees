...
# copy data to EnumerationPath table
ActiveRecord::Base.connection.execute(
    'INSERT INTO enumeration_path_employees(id,name,level,parent_id,technology,role) SELECT id,name,level,superior_id,technology,role FROM adjacency_list_employees')
# extra secret sauce for closure table model
EnumerationPathEmployee.build_ancestry_from_parent_ids!
